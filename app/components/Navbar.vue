...
<template>
  <header
    class="sticky top-0 z-50 border-b border-stone-200/80 bg-[#f7f7f2]/95 backdrop-blur-md"
  >
    <div
    ...
    >
      ...
      <NuxtLink
        to="/"
        class="group flex items-center gap-2 font-serif text-xl font-bold tracking-tight text-stone-900"
        @click="closeMobileMenu"
      >
        <span
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-800 text-xs font-sans font-bold text-white shadow-sm transition-transform duration-200 group-hover:scale-105"
        >
          R+
        </span>
      ...
      <div class="hidden items-center gap-2 sm:gap-3 md:flex">
        ...
        <div class="ml-1 border-l border-stone-300 pl-2">
          ...
          <template v-else>
            <div
              class="flex items-center gap-2 rounded-full border border-stone-300 bg-white p-1 pr-3 shadow-sm"
            >
              <NuxtLink to="/profile" class="flex items-center gap-2">
                <span
                  class="flex h-7 w-7 items-center justify-center overflow-hidden rounded-full bg-emerald-800 text-xs font-bold text-white"
                >
                  <img
                ...
                <span
                  class="max-w-[90px] truncate text-xs font-medium text-stone-800"
                >
                  {{ user.name }}
                </span>
              </NuxtLink>
    ...
    >
      ...
      >
        <!-- Mobile Search Input -->
        <form
          class="mb-4 flex items-center gap-2 rounded-full border border-stone-300 bg-white px-3 py-2"
          @submit.prevent="searchBooks"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <nav class="flex flex-col gap-1">
          <NuxtLink
            ...
            :class="
              isActive(item.path)
                ? 'bg-emerald-800/10 font-semibold text-emerald-800'
          ...
          >
            ...
            <span
              v-if="wishlistCount > 0"
              class="rounded-full bg-emerald-800 px-2 py-0.5 text-[10px] font-bold text-white"
            >
              {{ wishlistCount }}
            </span>
          </NuxtLink>
        ...
        <div
          v-else
          class="flex items-center justify-between rounded-xl border border-stone-200 bg-white p-2.5 shadow-sm"
        >
          <NuxtLink
            to="/profile"
          ...
          >
            <span
              class="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-emerald-800 text-xs font-bold text-white"
            >
              <img
                v-if="user.profileImage"
                :src="user.profileImage"

...
const isActive = (to: string) =>
  to === "/admin" ? route.path === to : route.path.startsWith(to);
...
<template>
  ...
  >
    <div
      class="flex h-20 items-center justify-between border-b border-[#e2ece8] px-6"
    >
      <NuxtLink
        to="/admin"
      ...
      >
        <span
          class="flex h-10 w-10 items-center justify-center rounded-xl bg-primary font-bold text-white shadow-lg shadow-primary/20"
          >R+</span
        >
        <span
          ><strong class="block font-serif text-lg text-ink">ReadPlus</strong
          ><small class="text-xs text-gray-500">Admin workspace</small></span
        >
      </NuxtLink>
      <button
        class="rounded-lg p-2 text-gray-500 hover:bg-[#eaf6f3] lg:hidden"
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6">
      <p
        class="mb-3 px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400"
      >
        Workspace
      </p>
      <NuxtLink
    ...
    <div class="border-t border-[#e2ece8] p-4">
      <NuxtLink
        to="/"
        class="mb-3 block rounded-xl border border-[#d9e7e2] px-4 py-3 text-sm font-semibold text-gray-600 hover:border-primary hover:text-primary"
        >View storefront</NuxtLink
      >
      <button
        class="w-full rounded-xl px-4 py-3 text-left text-sm font-semibold text-red-600 hover:bg-red-50"
        @click="logout"
      >
        Sign out
      </button>

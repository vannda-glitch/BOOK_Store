...
const increaseQuantity = () => {
  if (book.value && quantity.value < Math.max(0, Number(book.value.stock) || 0))
    quantity.value++;
};
...
const handleAddToCart = async () => {
  if (!book.value) return;
  if (
    Number(book.value.stock) === 0 ||
    quantity.value > Number(book.value.stock)
  ) {
    cartLimitMessage.value =
      Number(book.value.stock) === 0
        ? "This book is out of stock."
        : `Only ${book.value.stock} copies available.`;
    return;
  }
  for (let i = 0; i < quantity.value; i++) {
    const result = await addToCart(book.value);
    if (!result?.success) {
      cartLimitMessage.value = result?.message || "Unable to add this book.";
      return;
    }
  }
  cartLimitMessage.value = "";
...
<template>
  <div class="min-h-screen bg-[#fafafa] text-gray-900">
    <!-- Loading -->
    <main v-if="loading" class="mx-auto max-w-300 px-5 py-10 md:px-8 lg:px-10">
    ...
    >
      <div class="max-w-md text-center">
        ...
        >
          ...
          >
            <path
              ...
              d="M12 9v3.75m0 3.75h.007M10.29 3.86 2.82 17.25a1.5 1.5 0 0 0 1.3 2.25h15.76a1.5 1.5 0 0 0 1.3-2.25L13.71 3.86a1.5 1.5 0 0 0-2.42 0Z"
            />
          </svg>
        </div>
        <h1 class="mt-5 font-serif text-3xl font-bold text-gray-950">
          Book not found
        </h1>
        ...
        <NuxtLink
          to="/browse"
          class="mt-7 inline-flex items-center gap-2 rounded-xl bg-black px-6 py-3 text-xs font-semibold text-white transition hover:bg-gray-800"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
    ...
    <!-- Product Detail -->
    <main v-else class="mx-auto max-w-300 px-5 py-6 md:px-8 lg:px-10 lg:py-10">
      <!-- Breadcrumb -->
      <nav
        class="mb-8 flex items-center gap-2 text-[10px] font-medium text-gray-400"
      >
        <NuxtLink to="/" class="transition hover:text-black">Home</NuxtLink>
        <span>/</span>
        <NuxtLink to="/browse" class="transition hover:text-black"
          >Browse</NuxtLink
        >
        <span>/</span>
      ...
      <section class="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-16">
        ...
        <div>
          <div
            class="group relative overflow-hidden rounded-3xl border border-gray-100 bg-[#f1f2f5] shadow-sm"
          >
            <span
              v-if="Number(book.rating) >= 4.8"
              class="absolute left-5 top-5 z-10 rounded-full bg-white/95 px-4 py-2 text-[9px] font-semibold uppercase tracking-wider text-gray-800 shadow-sm backdrop-blur"
            >
              Highly Rated
            </span>
          ...
          <div class="mt-4 grid grid-cols-3 gap-3">
            ...
            >
              ...
              <p class="mt-1 truncate text-xs font-semibold text-gray-900">
                {{ book.category || "N/A" }}
              </p>
            </div>
            <div
        ...
        <div class="flex flex-col justify-center">
          ...
          <!-- Title -->
          <h1
            class="mt-5 max-w-xl font-serif text-4xl font-bold leading-tight tracking-tight text-gray-950 sm:text-5xl"
          >
            {{ book.title }}
          </h1>
          ...
          <div class="mt-6 flex items-center gap-3">
            <div class="flex items-center gap-0.5 text-sm">
              <span
                v-for="star in 5"
                :key="star"
                :class="star <= ratingStars ? 'text-gray-900' : 'text-gray-300'"
                >★</span
              >
            </div>
          ...
          <StockStatus :stock="book.stock" class="mt-4" />
          ...
          <div>
            ...
            <div class="mt-2 flex items-end gap-3">
              <span class="text-3xl font-bold tracking-tight text-gray-950"
          ...
          <div class="mt-8">
            <div class="flex flex-col gap-3 sm:flex-row">
              <!-- Quantity -->
              <div
                class="flex h-13 items-center rounded-xl border border-gray-200 bg-white"
              >
                <button
                  type="button"
                  class="flex h-full w-12 items-center justify-center text-gray-500 transition hover:text-black"
              ...
              <button
                ...
                class="flex h-13 flex-1 items-center justify-center gap-2 rounded-xl bg-black px-6 text-xs font-semibold text-white transition-all duration-200 hover:bg-gray-800 active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-gray-300"
            ...
            <p v-if="cartLimitMessage" class="mt-3 text-xs text-amber-700">
              {{ cartLimitMessage }}
            </p>
            <button
              type="button"
              class="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-primary px-6 text-xs font-semibold text-primary transition hover:bg-[#edf8f5]"
      ...
      <section class="mt-20 border-t border-gray-200 pt-12">
        <div class="max-w-3xl">
          ...
          <h2 class="mt-2 font-serif text-3xl font-bold text-gray-950">
            {{ book.shortTitle || book.title }}
          </h2>
          <p class="mt-5 text-sm leading-7 text-gray-500">
            Discover
            <span class="font-medium text-gray-700">{{ book.title }}</span> by
            <span class="font-medium text-gray-700">{{ book.author }}</span
            >. This book is part of our
            <span class="font-medium text-gray-700">{{ book.category }}</span>
            collection and is available in
            <span class="font-medium text-gray-700">{{ book.format }}</span>
            format.
          </p>
        </div>
      </section>
      ...
      <section
        ...
        class="mt-20 border-t border-gray-200 pt-12"
      >
        <div class="flex items-end justify-between">
          ...
          <NuxtLink
            to="/browse"
            class="hidden items-center gap-2 text-xs font-semibold text-gray-500 transition hover:text-black sm:flex"
          >
            View all
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div
              class="overflow-hidden rounded-2xl border border-gray-100 bg-[#f1f2f5]"
            >
            ...
            <div class="pt-4">
              <div class="flex items-start justify-between gap-4">
                ...
                <span class="shrink-0 text-sm font-bold text-gray-950"
                  >${{ Number(related.price).toFixed(2) }}</span
                >
              </div>
              <div class="mt-3 flex items-center gap-2">
                <div class="text-[11px]">
                  <span
                    ...
                    :key="star"
                    :class="
                      star <= Math.round(Number(related.rating))
                        ? 'text-gray-900'
                        : 'text-gray-300'
                    "
                    >★</span
